...
class GoogleDriveDownloader {
    ...
    private async downloadFilesInBatches(allFileItems: Array<{ file: any; folderPath: string }>, folderOutputDir: string): Promise<void> {
        const batchSize = 25;
        ...
        for (let i = 0; i < allFileItems.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async ({ file, folderPath }, batchIndex) => {
                ...
                console.log(`${progress} Processing: ${path.join(folderPath, file.name)}`);
